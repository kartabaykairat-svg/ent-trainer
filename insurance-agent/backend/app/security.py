...
def get_fernet() -> Fernet:
    ...
    _fernet = Fernet(_to_fernet_key(key))
    return _fernet


def _to_fernet_key(raw: str) -> bytes:
    """Fernet needs a 32-byte urlsafe-base64 key. Accept one directly (e.g.
    from `Fernet.generate_key()`), but also accept any other secret string
    (e.g. a hosting platform's auto-generated env var, which is a random
    password-shaped string, not base64) by deriving a valid key from it via
    SHA-256 - so any sufficiently random FIELD_ENCRYPTION_KEY value works,
    not just one generated by this exact library."""
